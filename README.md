...
This solves the problem of monitoring services that are behind a firewall and are not accessible from the internet. It uses the external monitoring feature of your monitoring service to send alerts to your monitoring service.
...
- **Global Initialization Delay**: Integrated grace period to allow services (like VPNs) to settle before monitoring
- **Relative Time Checks**: Verify if a timestamp is within a specific age (e.g., `< 10m`)
- **Flexible Configuration**: YAML-based config with auto-reload and global defaults
- **Target Modes**: Monitor multiple targets with `any` (failover) or `all` (cluster) modes
- **TLS Optimization**: Native Go cross-compilation for multi-architecture Docker builds
...
- **Example**:
  ...
  - name: "Web Service"
    ...
    monitor_endpoint:
      success:
        url: "https://uptime.kfalabs.com/api/push/success?duration={%duration%}ms"
      ...
      failure: # Optional failure endpoint. Useful to send error messages to an alert endpoint.
        url: "https://uptime.kfalabs.com/api/push/failure?error={%error%}"
  ```
...
- **Global Docker Sockets**: These are used to connect to Docker daemons. You can specify multiple sockets for different environments or configurations.
...
**Note**: The global `monitor_endpoint` supports only the `headers` field. Success and failure URLs must be configured at the service level.
...
Releases are triggered **manually** via the GitHub Actions tab.

1.  Navigate to the **Release** workflow in GitHub Actions.
2.  Click **Run workflow**.
3.  Specify the version bump type (`patch`, `minor`, `major`).

This will:
1.  Run all tests.
2.  Automatically calculate and create a new semantic version tag.
3.  Build optimized Docker images for `linux/amd64`, `linux/arm64`, and `linux/arm/v7` using **native Go cross-compilation** (no slow QEMU emulation).
4.  Push images to Docker Hub and GHCR with appropriate tags (`latest`, `v1.x.x`, etc.).
5.  Create a GitHub Release with auto-generated notes and build attestations.
