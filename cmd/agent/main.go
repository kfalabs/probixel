...
func main() {
	...
	for {
		...
		for _, svc := range currentCfg.Services {
			probe, err := setupProbe(svc, currentCfg.Global)
			if err != nil {
...
func setupProbe(svc config.Service, globalCfg config.GlobalConfig) (monitor.Probe, error) {
	...
	if dockerProbe, ok := probe.(*monitor.DockerProbe); ok && svc.Docker != nil {
		dockerProbe.Sockets = globalCfg.DockerSockets
